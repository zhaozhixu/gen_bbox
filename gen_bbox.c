...
struct pre_alloc_tensors *gb_preprocess(void)
{
     ...
     tensors->feature = tl_tensor_create(NULL, 3, dims_feature, TL_INT16, NULL);
     int dims_conf_feature[] = {CONF_SLICE_C,CONVOUT_H,CONVOUT_W};
     tensors->conf_feature = tl_tensor_create(NULL, 3, dims_conf_feature, TL_INT16, tensors->feature);
     int dims_bbox_feature[] = {BBOX_SLICE_C,CONVOUT_H,CONVOUT_W};
     tensors->bbox_feature = tl_tensor_create(NULL, 3, dims_bbox_feature, TL_INT16, tensors->feature);
     ...
     tl_tensor *anchor_shapes = tl_tensor_create(ANCHOR_SHAPES, 2, dims_create_anchor, TL_FLOAT, NULL);
